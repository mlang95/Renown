...
rangesforestswetlandstundraregionsresources on the hexmap territory
...
def _page_svg(window, hexes, R, page_wmm, page_hmm, margin)
    ...
    txp = margin - bx0
    typ = margin - by0
    parts = [f'svg xmlns=httpwww.w3.org2000svg '
             f'width={page_wmm}mm height={page_hmm}mm '
             f'viewBox=0 0 {page_wmm} {page_hmm}',
             f'clipPath id=cwrect x={margin} y={margin} '
    ...
    if settlements
        ...
        for settles in settlements
            ...
            parts.append(f'polygon points={poly} fill=none stroke=#c01616 '
                         f'stroke-width={sw.2f} stroke-dasharray={R0.2.1f} {R0.15.1f}')
            cx0, cy0 = pts[0]
...
def generate_tactical_board(width=9, height=6, , seed=7, out_pdf=tactical_board.pdf,
    ...
    hills tile, no labelstrim grid (single page). Auto-clamps widthheight so the
    whole board fits one landscape page at hex_mm.
    if paper not in PAPER
        raise ValueError(fpaper must be one of {list(PAPER)})
...
def _coerce_map(src)
    HexMap  exported dict  path-to-json - HexMap.
    if hasattr(src, all) and hasattr(src, width)
        return src
    ...
    if isinstance(src, dict)
        ...
        return m
    raise TypeError(fcan't read a board from {type(src).__name__})
...
def generate_board(width=None, height=None, , seed=7, place_resources=True,
    ...
    map              a pre-built board instead of generating one. Accepts a
                      HexMap, an exported-map dict, or a path to an exported
                      JSON file. This is how a board made elsewhere — a
    ...
    if map is not None
        ...
        if width is None or height is None
            raise ValueError(give width and height, or pass map=)
        mp = dict(mapgen_params or {})
        m = mapgen.generate(width=width, height=height, seed=seed, mp)

    # page footer describes the board that was actually printed. A board built
    # elsewhere carries its own presetseedgenerator; stamping the local
    # `seed` argument on it would be a quiet lie.
    meta = getattr(m, meta, None)
    if meta
        board_id = (f{meta.get('preset','')}  {width}x{height}  
                    fseed={meta.get('seed','')}  gen={meta.get('generator','')})
    elif map is not None
        board_id = f{width}x{height}  (imported)
    else
        board_id = f{width}x{height}  seed={seed}
    ...
    for pr in range(prows)
        for pc in range(pcols)
            ...
            tmp = os.path.join(_TMPDIR, f_pg_{pr}_{pc}.svg)
            with open(tmp, w) as f
                f.write(svg)
            renderPDF.draw(svg2rlg(tmp), c, 0, 0)
            label = (fRenown  {board_id}   
                     fpage col {pc} row {pr}  (of {pcols}x{prows}))
