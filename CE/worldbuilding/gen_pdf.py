...
PAGE      = A4
MARGIN    = 18 * mm
# map image: RENOWN_MAP (set by build_all_CE.bat), else map7.png beside this
# file. Blank or missing = the book builds without the map page image.
MAP_FILE  = os.environ.get("RENOWN_MAP") or os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "map7.png")
...
def map_page(w, h):
    ...
    _map_ok = bool(MAP_FILE) and os.path.exists(MAP_FILE) and os.path.getsize(MAP_FILE) > 1024
    if MAP_FILE and not _map_ok:
        print(f"  map skipped - {MAP_FILE} missing or an LFS pointer")
    if _map_ok:
