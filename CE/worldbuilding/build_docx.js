...
// file. >1024 bytes rules out a Git-LFS pointer masquerading as the image.
const MAP = process.env.RENOWN_MAP || path.join(__dirname, 'map7.png');
const hasMap = MAP && fs.existsSync(MAP) && fs.statSync(MAP).size > 1024;
...
if (hasMap) {
  sect.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:200},
    children:[new ImageRun({type:'png',data:fs.readFileSync(MAP),
