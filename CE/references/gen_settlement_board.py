...
# Latin + Latin-1/Ext-A, Greek (Σ), general punctuation, arrows, math (− ×), box/geometric shapes
_FONT_RANGES = [(0x20, 0x24F), (0x370, 0x3FF), (0x2000, 0x206F), (0x20A0, 0x20BF),
                (0x2100, 0x214F), (0x2190, 0x21FF), (0x2200, 0x22FF), (0x25A0, 0x25FF), (0x2600, 0x26FF)]

def font_faces():
    """@font-face rules with EB Garamond embedded (subset, woff) so the board stays
    self-contained/offline. Missing fonttools or font files -> empty string (falls back to Georgia)."""
    try:
        import base64, io
    ...
    for fname, style in (("EBGaramond-VariableFont_wght.ttf", "normal"),
        ...
        f = TTFont(path)
        opt = Options(); opt.flavor = "woff"; opt.layout_features = ["*"]
...
/*__FONTS__*/
  ...
  *{box-sizing:border-box}
  ...
  details.section>summary{list-style:none;cursor:pointer;padding:8px 12px;display:flex;
    align-items:center;gap:8px;font-family:var(--serif);font-size:15px;border-bottom:1px solid var(--line)}
  ...
  .dragging{opacity:.35}
  .dragghost{position:fixed;z-index:1000;pointer-events:none;background:var(--panel);border:1px solid var(--line2);border-radius:var(--radius);padding:5px 8px;box-shadow:0 8px 22px rgba(0,0,0,.5);font-family:var(--serif);font-size:12px;max-width:260px}
  ...
  .stat b{color:var(--ink);font-family:var(--serif)}
  ...
  .scell .sn{color:var(--dim);font-size:10px}.scell .sv{font-family:var(--serif);font-size:16px}
  ...
  .treas input{width:120px;font-family:var(--serif);font-size:16px}
  ...
  .vtab{padding:6px 12px;border:1px solid var(--line);border-radius:var(--radius);background:var(--panel2);color:var(--dim);cursor:pointer;font-family:var(--serif)}
  ...
  .dtable th{color:var(--dim);font-weight:600;font-family:var(--serif)}
  ...
  .dtrack .dname{width:78px;font-family:var(--serif);color:var(--dim);display:flex;align-items:center;font-size:13px}
...
function standingsBoardHTML(){
  ...
  h+='<div style="margin-top:8px"><div style="color:var(--dim);font-family:var(--serif);margin-bottom:3px">Renown (shared): '+(D.renown||1)+' · Era '+era+'</div>'+
...
function renderRenown(){
  ...
  DOMAINS.forEach(d=>{const v=(ab.domains||{})[d]||0;
    h+='<div style="margin-bottom:8px"><div style="color:var(--dim);font-family:var(--serif);margin-bottom:2px">'+d+' <span class="note">(cube '+v+' · '+domBand(v)+')</span></div>';
