...
let D={players:[],active:0,view:"board",renown:1,theme:"ink",
...
function normalizeD(){
  ...
  D.theme=D.theme||"ink"; D.shape=D.shape||"rounded"; D.pursuitView=D.pursuitView||"cards";
...
function render(){
  ...
  applyTheme(D.theme||"ink");
  applySkin(D.shape||"rounded");
  renderTopBar();
...
function renderTopBar(){
  ...
  if(ts){ts.value=D.theme||"ink";if(!ts._wired){ts._wired=1;ts.onchange=()=>{D.theme=ts.value;applyTheme(D.theme);save();render();};}}
  const sh=document.getElementById("shapeSel");
  if(sh){sh.value=D.shape||"rounded";if(!sh._wired){sh._wired=1;sh.onchange=()=>{D.shape=sh.value;applySkin(D.shape);save();render();};}}
