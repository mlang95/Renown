...
class StaticArmy:
    ...
    def __init__(self, loadout, is_attacker):
        ...
        self.max_init = 3 if ("MaxInit3" in ld.extra_tags
                              or any(str(t).startswith("Crit ") for t in ld.extra_tags)) else 2
        ...
        if _allowed:
            self.tactic_mask_ranged = np.array(
                [t in _allowed for t in TACTICS], dtype=bool)
        else:
            self.tactic_mask_ranged = None
        # Restriction applies on skirmish 1 whenever the ranged weapon is up, and on
        # later skirmishes only while it keeps firing (pure-ranged multi-shot).
        self.tactic_restricted_first = self.tactic_mask_ranged is not None and self.uses_ranged_first
        self.tactic_restricted_normal = self.tactic_mask_ranged is not None and self.uses_ranged_normal
...
def _crit_floor(tags):
    """Focused threshold for this attacker.

    Reads any `Crit N` tag (Crit 9, Crit 8, ...) so Ministry can be retuned in
    renown_data without an engine change; the lowest N present wins. Falls back to
    the global FOCUSED_THR when no tag is carried.
    """
    floors = []
    for t in tags:
        m = re.match(r"^Crit (\d+)$", str(t))
        if m:
...
def _roll_strikes_vec(rng, n, target_th, front_line, atk_tags, defender_has_shield_flag,
                     ...
                     defender_shield_immune=False, atk_crit_floor=None):
    ...
    """
    if atk_crit_floor is None:
        atk_crit_floor = FOCUSED_THR
...
def _counter_weights_from_table(counter_tbl, opp_tac_idx, n_runs, counter_weight=0.8):
    ...
    n_tac = counter_tbl.shape[0]                 # tactic count (7), NOT dice faces
    weights = np.full((n_runs, n_tac), (1 - counter_weight) / float(n_tac - 1), dtype=np.float64)
...
def _roll_saves_vec(rng, n, save_target, n_strikes, shatter_strikes, atk_has_poison, def_has_parry, def_regen_threshold, def_has_regen_reroll=False, atk_unstoppable=False, def_has_riposte=False, def_parry_improved=False, def_can_parry_shatter=False, atk_is_ranged=False, def_fat=None, def_planishing=False, def_crit5=False, atk_has_deflect=False, atk_has_halfsword=False, atk_ignores_tempered=False):
    ...
    parry_rolls = rng.integers(1, FACES + 1, size=(n, max_strikes), dtype=np.int8) if any_parry \
        else np.zeros((n, max_strikes), dtype=np.int8)
    regen_rolls = rng.integers(1, FACES + 1, size=(n, max_strikes), dtype=np.int8) if any_regen \
        else np.zeros((n, max_strikes), dtype=np.int8)
...
def _renorm_tactics(weights):
    """Renormalize per-run tactic weights to sum to 1 after some have been zeroed.

    sample_tactic_indices() pins cumw[:, -1] = 1.0, which makes the final bin
    (Fall Back) a catch-all for any missing probability mass. Without this
    renormalization, zeroing a tactic hands its weight to Fall Back rather than
    redistributing it across the tactics that are still legal. Rows that end up
    entirely zero fall back to a uniform draw so a run can never stall.
    """
...
def run_matchup_vec(ld_a, ld_b, n_runs=100, max_skirmishes=20, seed=None, alternate_attacker=True,
    ...
    for sk in range(max_skirmishes):
        ...
        # NOTE on zeroing tactic weights: sample_tactic_indices() forces
        # cumw[:, -1] = 1.0, so the LAST bin (Fall Back) absorbs whatever
        # probability mass is missing. Zeroing any weight without renormalizing
        # therefore silently redirects that mass into Fall Back. Always pass
        # masked weights through _renorm_tactics() before sampling.
        _tactics_masked = False
        if sk < fb_min_skirmish:
            a_weights = a_weights.copy(); a_weights[:, FB_IDX] = 0.0
            b_weights = b_weights.copy(); b_weights[:, FB_IDX] = 0.0
            _tactics_masked = True

        # Weapon-imposed tactic restriction (Arquebus): while the restricted weapon
        # is the active one, zero every disallowed tactic.
        _a_restrict = (a_static.tactic_restricted_first if first
                       else a_static.tactic_restricted_normal)
        _b_restrict = (b_static.tactic_restricted_first if first
                       else b_static.tactic_restricted_normal)
        if _a_restrict:
            a_weights = a_weights.copy()
            a_weights[:, ~a_static.tactic_mask_ranged] = 0.0
            _tactics_masked = True
        if _b_restrict:
            b_weights = b_weights.copy()
            b_weights[:, ~b_static.tactic_mask_ranged] = 0.0
            _tactics_masked = True

        if _tactics_masked:
            a_weights = _renorm_tactics(a_weights)
            b_weights = _renorm_tactics(b_weights)
        ...
        a_th_pre = np.minimum(a_static.to_hit + a_th_improve + a_fat, CAP_THR)
        b_th_pre = np.minimum(b_static.to_hit + b_th_improve + b_fat, CAP_THR)
        ...
        # NOTE: fatigue-token accrual now happens AFTER the shaking test below...
