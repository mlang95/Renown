...
import build_monument_paths as mp
...
S = dict(
    ...
    b=ParagraphStyle("b", fontName="EBGaramond", fontSize=8.4, leading=10.2, textColor=ink),
    step=ParagraphStyle("step", fontName="EBGaramond", fontSize=8.4, leading=10.4, textColor=ink, spaceAfter=2, leftIndent=10, firstLineIndent=-10),
...
story.append(P("A <b>natural</b> roll is the die before modifiers. A <b>natural 6</b> triggers <b>Cleave, Shatter Armor, Destroy "
...
story.append(P("VICTORY", "h2"))
story.append(P("After <b>10 Battles</b>, highest VP wins. <b>Tie:</b> keep playing full turns until one tied player <b>leads by 3 VP</b>."))
story.append(P("EDICTS (+1 VP each)", "h2"))
story.append(P("Gain <b>+1 VP</b> every time you reach a <b>Sovereign</b> Standing in any Domain, or <b>complete (master) a Monument</b>: "
               "<i>Advanced Blast Furnace, Royal Pavilion, Preceptory of the Knight's Templar, Ministry of Military Strategy, Outrider Intercept Post.</i>"))
story.append(P("BUILDING", "h2"))
story.append(P("A Build Action constructs <b>one Pursuit</b>; the only requirement is its <b>Build Action Requirement</b>. Its <b>Innate</b> effect applies at once; its <b>Mastery</b> effect once every <b>Pursuit</b> in its <b>Mastery Requirement</b> is also built. You may freely re-equip before each Battle with anything unlocked."))
...
story.append(btable(T_PROW, [
 ...
 ("Ministry of Military Strategy", "Sovereign Prowess", "University, War College", "Seize the Initiative in Skirmish 1 (the enemy never can); ignore every Tactic to-Strike penalty", "Gain +1 Initiative and +1 to Strike each Skirmish; maximum Initiative raised by 1 (to +3)", True),
]))
...
# ── NON-COMBAT ENABLERS (with Purpose) + GLOSSARY ──
story.append(P("NON-COMBAT ENABLERS", "h2"))
story.append(P("<i>No battle effect of their own — built only as prerequisites or mastery requirements.</i>", "bsm"))
_en = [("Pursuit", "Build Action Req", "Purpose", "Feeds into"),
   ("Carpentry", "—", "<b>Build Timer −1</b>; +100; <b>Craft +1</b>", "Joinery, Fletchery"),
   ("Animal Husbandry", "—", "+400, <b>Natural</b>", "Tannery, Stable"),
   ("Levy Hall", "Rising Prowess", "<b>Upkeep −200</b>", "War College"),
   ("Academy", "—", "<b>Influence 1</b> to <b>Council Envoys</b>", "War College, Ministry, Toxicarium"),
   ("University", "—", "+1 <b>Influence</b> per <b>Domain</b> you are <b>Rising</b>", "Ministry, Cipher Chamber"),
   ("Herb Garden", "—", "+200, <b>Natural</b>; <b>Efficient Hamlet</b>", "Apothecary, Infirmary"),
   ("Alchemy", "—", "+100; <b>Craft +1</b>", "Apothecary, Infirmary"),
   ("Courtyard", "—", "+200; <b>Faith 1</b>", "Conditioning Field"),
   ("Monastery", "Established Piety", "Other players can't <b>Oppose</b> your <b>Piety Envoys</b>; <b>Efficient Abbey</b>", "Preceptory (mastery)"),
   ("Pilgrimage Site", "Rising Piety", "<b>Doubt 1</b> to all other players without a Pilgrimage Site; <b>Efficient Reliquary</b>", "Preceptory (mastery)"),
   ("Abbey", "Rising Piety", "Once/turn: <b>Influence 1</b> another player's <b>Piety Envoy</b>", "Preceptory (mastery)"),
   ("Caravanery", "—", "<b>Influence −1</b> to <b>Intercept Caravan</b> actions targeting your <b>Settlements</b>; <b>Craft +2</b>", "Outrider Intercept Post"),
   ("Cipher Chamber", "Sovereign Cunning", "Once/turn, select any <b>active timer</b> you did not select last turn: increase the timer +1", "Outrider Intercept Post")]
_enrows = [[Paragraph(c, S["tcb"] if i == 0 else S["tc"]) for c in row] for i, row in enumerate(_en)]
story.append(tbl(_enrows, [W*0.19, W*0.27, W*0.26, W*0.28]))
...
G = [
 ...
 ("Natural roll", "The number on the die before any modifiers. A natural 6 triggers Cleave, Shatter Armor, Destroy Shield and Riposte — modifiers never change what counts as 'natural'."),
 ...
 ("Parry", "Roll a d6 to cancel a Strike on a 5+ (a natural 6 is a Riposte). −1 versus Unstoppable, versus ranged, and per Fatigue token (to a maximum of 6+). Shatter Armor can only be Riposted."),
 ("Improved Parry", "Your Parry succeeds on 4+ instead of 5+."),
 ("Riposte", "A Parry of a natural 6 cancels the Strike and Strikes back once; it is also the only way to Parry a Shatter Armor Strike."),
 ...
 ("Recover X", "After a failed Parry and Save, roll a d6: a result of X+ saves the retinue (worsened by Fatigue and Serrated, to a maximum of 6+). Against a Shatter Armor Strike, only a natural 6 saves."),
 ...
 ("Shatter Armor", "A natural 6 to Strike skips the armor Save entirely; only a Riposte or a natural-6 Recover can stop it."),
...
def glossary_table(entries, width):
    gs = ParagraphStyle("gl", fontName="EBGaramond", fontSize=6.9, leading=7.9, textColor=ink, spaceAfter=2.4)
    ents = sorted(entries, key=lambda e: e[0].lower())
    half = (len(ents) + 1) // 2
    col = lambda items: [Paragraph(f"<b>{t}</b> — {d}", gs) for t, d in items]
    cw = (width - 12) / 2
...
# ════════════════════ MONUMENT BUILD PATHS — one landscape sheet (2-col grid) ════════════════════
DM = 0.3 * inch
DW = LPAGE[0] - 2 * DM
ddoc = BaseDocTemplate("diagrams.pdf", pagesize=LPAGE, leftMargin=DM, rightMargin=DM, topMargin=DM, bottomMargin=DM)
ddoc.addPageTemplates([PageTemplate(id="land", frames=[Frame(DM, DM, DW, LPAGE[1] - 2 * DM, id="df")])])
dstory = [P("MONUMENT BUILD PATHS", "h1"),
          P("Grey arrow = a Building you must build to master the one it points to. "
            "<b>Purple dashed arrow = required to master the Monument</b> (its Edict). "
            "Node colour = Domain; tag = the Standing it needs.", "sub")]
cellw = DW / 2
def dimg(Mn, boxw, maxh):
    fn = "mon_" + "".join(c for c in Mn if c.isalnum()) + ".png"
    mp.draw(Mn, fn)
    iw, ih = ImageReader(fn).getSize()
    w = boxw; h = w * ih / iw
    if h > maxh: h = maxh; w = h * iw / ih
    return RLImage(fn, width=w, height=h)
# Advanced Blast Furnace spans the full width (busiest chain)
abf = mp.MONUMENTS[0]
dstory += [Spacer(1, 4), P(mp.disp(abf), "h2"), dimg(abf, DW - 12, 2.4 * inch), Spacer(1, 6)]
# the other four monuments in a 2 x 2 grid
cells = [[P(mp.disp(Mn), "h2"), dimg(Mn, cellw - 12, 1.4 * inch)] for Mn in mp.MONUMENTS[1:]]
gridrows = [cells[0:2], cells[2:4]]
grid = Table(gridrows, colWidths=[cellw, cellw])
grid.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("ALIGN", (0, 0), (-1, -1), "LEFT"),
...
main = PdfReader("main.pdf"); board = PdfReader("board.pdf"); diagrams = PdfReader("diagrams.pdf"); tactics = PdfReader("tactics.pdf")
out = PdfWriter()
for pg in main.pages:                 # rules, armory, buildings, enablers + glossary
    out.add_page(pg)
out.add_page(diagrams.pages[0])        # monument build paths (landscape, one sheet)
...
print("pages:", len(PdfReader("ESCALATION_CAMPAIGN.pdf").pages),
      "| diagrams pages:", len(diagrams.pages), "| board pages:", len(board.pages))
