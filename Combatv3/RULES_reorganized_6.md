...
- **Influence** — the weight you bring to Envoy votes. Base Influence comes from your Standing (1/2/3/4); you may spend up to 1 + Standing to support or oppose other players' envoys of that Domain.
...
5. **Innate + Mastery are cumulative.**

6. **Damaged means inactive.** A Damaged Pursuit or Infrastructure has neither its innate nor its mastery effect.
...
—8.2. If a Roll is modified below 2+, it automatically passes.

—8.3. If modified to 1+ or 7+ but specific results trigger on a 1 or 6, you still roll the dice for those specific results.

—8.4. You can't re-roll a re-roll, and you can't re-roll a success.

—8.5. Effects that occur on a 1 or 6 occur on unmodified rolls of 1 or 6.

9. **Rounding (denomination of 100).**

—9.1. When a gold amount isn't divisible by 100, round down to the nearest 100, minimum 0.

—9.2. If your Treasury is negative, round up to the nearest 100 instead.

10. **PEMDAS.** When adding and multiplying, add all values first, then multiply. If several things multiply against the same sum, add the multiplication values together first.

11. **Do as much as possible.** Follow as much of an effect as you can; ignore any part you can't carry out.
...
14. **Ward exemption** applies only within the same **settlement ward** that is **efficient** with that ward.
...
**Timers.** Many actions set a *Timer* to a number of turns. Each Empire Phase, every active Timer ticks down by 1; when a Timer reaches 0 it resolves and is removed (a Timer set to 0 or less resolves immediately). Each action states its own Timer's length — the named types (Build, Repair, Muster, Siege, Sack, Capture, Convert, Truce) are just labels for *what* resolves; the rule is the same for all of them.
