...
# ---------- Data ----------
# Tactic order + matrix come from renown_data — the single source of truth.
# Edit renown_data.TACTIC_MATRIX to rebalance; the display strings below are generated.
from renown_data import TACTICS as TACTIC_ORDER, TACTIC_MATRIX
...
def _cell_text(m):
    ...
    parts = []
    if m["I"]:
        parts.append(f"+{m['I']}I" if m["I"] > 0 else f"{m['I']}I")
    if m["TH"]:
        parts.append(("+1" if m["TH"] > 0 else "-1") + " to Strike"
                     if abs(m["TH"]) == 1 else
                     (f"+{m['TH']}" if m["TH"] > 0 else str(m["TH"])) + " to Strike")
    if m["TS"]:
        parts.append(("+1" if m["TS"] > 0 else "-1") + " to Save"
                     if abs(m["TS"]) == 1 else
                     (f"+{m['TS']}" if m["TS"] > 0 else str(m["TS"])) + " to Save")
    return ", ".join(parts) if parts else "No Bonus"


# Rows = your tactic. Columns follow TACTIC_ORDER. Generated from TACTIC_MATRIX.
TACTIC_TABLE = {
    mine: [_cell_text(TACTIC_MATRIX[(mine, theirs)][0]) for theirs in TACTIC_ORDER]
    for mine in TACTIC_ORDER
}
...
RISK_REWARD = {
    "Scout":               ("Exposed",   "Quick"),
    "Ambush":              ("Clumsy",    "Quick & Fortified"),
    "Flank":               ("Slow",      "Quick & Precise"),
    "Charge":              ("Slow & Exposed",   "Quick & Precise"),
    "Fighting Formation":  ("Slow",      "Precise"),
    "Defensive Formation": ("Slow",      "Fortified"),
    "Fall Back":           ("Slow & Strained",      "Survive"),
}
...
def _layout_row(opp, outcome, font_left, font_right, size, max_w, gap=2, min_dots_w=4):
    ...
    Decide how to lay out a row. Returns list of line-segments:
      ...
      [{'left': 'Defensive Formation', 'right': '+1I, +1 to Save,'},
       {'left': '',                    'right': '-1 to be Strike'}]   # wrapped
    """
...
def make_pdf(pdf_path, copies=2):
    c = canvas.Canvas(pdf_path, pagesize=letter)
    per_page = COLS * ROWS

    # Each tactic printed `copies` times (default 2) and laid out contiguously,
    # so the deck packs tight with no half-empty trailing page per tactic.
    deck = [t for t in TACTIC_ORDER for _ in range(copies)]
    for i, tactic in enumerate(deck):
        slot = i % per_page
        if slot == 0 and i > 0:
            draw_crop_marks(c)
            c.showPage()
    ...
    draw_crop_marks(c)
    c.save()
    print(f"Wrote {pdf_path}  ({len(deck)} cards, "
          f"{(len(deck) + per_page - 1) // per_page} page(s))")
