...
def pack_side(loadouts, is_attacker):
    ...
    for t in ["Cleave", "Destroy Shield"]:
        ...
        P[f"first_{key}"]  = np.array([t in a.tags_first  for a in armies], dtype=np.bool_)
        P[f"normal_{key}"] = np.array([t in a.tags_normal for a in armies], dtype=np.bool_)
    # Per-slot crit_floor (proc threshold): 4 if Crit 4, 5 if Crit 5, else 6 (natural-6 only).
    def _crit_floor(tagset):
        if "Crit 4" in tagset: return 4
        if "Crit 5" in tagset: return 5
        return 6
...
def _roll_strikes_batch(rng, n, target_th, front_line, has_deadly, has_cleave,
    ...
    per-die logic to vectorized_combat. Per-slot arrays (n,):
      ...
      has_destroy : attacker can Destroy Shield (proc on natural 6 only)
      crit_floor  : 6 (natural-6 only) / 5 (Crit5) / 4 (Crit4) — proc threshold per slot
    ...
    target_th_c = np.clip(target_th, 2, 7).astype(np.int64)
    auto_fail = (target_th_c >= 7)
    auto_pass = (np.asarray(target_th) < 2)
