...
Each turn follows through five phases and repeats:

1. **Empire Phase** — start of turn: activate Standing effects, apply the Season, increment timers, collect income, and pay upkeep.
...
- **Influence** — the weight you bring to Envoy votes. Base Influence comes from your Standing (1/2/3/4); you may spend up to 1 + Standing to support or oppose other player's envoys of that Domain.
...
- **Faith** — devotion, generated by mostly Piety pursuits; powers religious effects and offsets Doubt.
...
2. **Special rule priority**: More specific rules override general rules.
...
7. **Timer 0**: All timers whose initial value is 0 or less resolve immediately.

8. **Dice rules**

—8.1. If a Roll is modified beyond 6+, it automatically fails.

—8.2. If a Roll is modified less than 2+, it automatically passes.

—8.3. If modified to 1+ or 7+ but specific results trigger on a 1 or 6, dice must still be rolled for those specific results.

—8.4. You may not re-roll a re-roll or re-roll a success.

—8.5. Effects that occur on a 1 or 6 occur on unmodified rolls of 1 or 6.

9. **Rounding down / denomination of 100**

—9.1. Whenever a gold amount is not divisible by 100, round down to the nearest 100, minimum 0.

—9.2. if your Treasury is negative, round up to the nearest 100 instead.

10. **PEMDAS rule**: Whenever adding and multiplying: add all values first, then multiply. If multiple things multiply against the same sum, add the multiplication values together first.

12. **Transitive property**: The transitive property states that if one value equals a second, and the second equals a third, then the first and third must be equal.

13. **Influence Spending:** On a single vote, you may spend **influence** equal to 1 + your **domain** Standing in that **domain** (Untested = 1, Rising = 2, Established = 3, Sovereign = 4).

14. **Ward exemption** only applies when in the same **settlement ward** that is  **efficient** with that ward.

15. **Monument Pursuits** & **Wonders** are unique. Only one of each name can exist in each game.

# Setup

## Table Setup

- Domain Board, set Renown to 1, and set each Player's Standing in each Domain to 1. Set Season to Summer. Set Public Order to 0.
- Give Host Card to Player that wants to Host first. If multiple, roll-off.
- Hex Map
- Tactic Decks, Equipment & Retinue Cards, Pursuit Tiles, Bandit Camps, and all the various tokens. 
...
- **Host Resolves Bandit Mechanics:**
	...
	- Spawn new **bandit camps** (in Spring, from Foster Rebellion, or Uprising)
...
- **Income & Upkeep:**
	...
	- Determine Net Income for Wealth Edict Condition
- **Apply innate Public Order modifiers** — check each Faith/Doubt source in the Public Order tables and adjust your Public Order. (Faith/Doubt from actions are already applied the moment they are generated; see Public Order.)
...
You only spend influence when you **Support** or **Oppose**. Abstaining is free, and you may let as much pass unopposed or supported as you like — though other players may not like that. **You cannot vote on your own envoy.** Each player may spend up to **1 + their Standing** in that domain on a single vote (Untested 1, Rising 2, Established 3, Sovereign 4); pursuits, infrastructure, and factions may raise this. At the end of each turn, unused Influence is discarded.

In practice this is not a round-the-table poll. The host simply asks *"anyone support or oppose?"* and resolves. The clockwise order is only there to settle the rare case where who-spends-first changes a decision.

Why this is the heart of the game: at **Untested** your envoy starts at **1**, and you can't support it yourself — so it can easily fail if **another player chooses to oppose you** and saved if **another player chooses to support you**. Influence is the currency of those actions, and are scaled by your standing in that domain. Unless players back each other, envoys can fail from cheap opposition; deciding whose actions to lift and whose to bury is the core of every turn.

Add everything together for the **net influence**, then read the outcome:

{{TABLE:net_influence}}

*If a player has an effect that reduces a specific action's influence, that reduction is applied after the net influence is totaled, and can turn a would-be pass into a fail. That player may instead perform a different action of the same Domain targeting the same player or settlement.*
...
Each turn, gain **influence** based on the following sources:
...
Unless the target player is at War, **players** cannot **Oppose** **Industry envoys** from that player.
...
### Domain Standings
 	...
 	**Domains** are the identity of your character in **Renown**. As you gain **Renown** each turn, you’ll gain access to a **Domain point**, which you can spend in any of the four **Domains**. Every time you spend a **Domain point**, you increase your **Domain value** by 1 in that Domain. When your **Domain value** becomes *three*, you become **Rising** in that **Domain**, allowing you spend an additional **influence** on another **player’s envoy** of that **domain**, and your **envoys** gain **influence +1**. This continues when you have a **domain value** of *six*, causing you to become **Established** in that **domain**, and **Sovereign** when your reach a **domain value** of *ten*.

Each **standing** you gain in a **domain** makes you harder to oppose in that domain politically and allows you oppose others more easily with your influence. In addition, you gain a unique effect at each **standing** that makes it easier to achieve the objectives of that **domain**.
...
*Note: mutual exchange (empire phase of each turn): if both players agree, either player may exchange up to {{VAL:VASSAL_EXCHANGE_CAP}} gold, exchange ownership of settlements, or transfer forces (so long as armies are adjacent to each other or an army is adjacent to a garrison), in either direction.*
...
**Settlements** are the primary vehicle of your Empire. **Settlements** are separated into **tiers**: **villages**, **towns**, **cities**, and a **capital metropolis**. Each tier provides **tax income** each **turn**, **settlement wards** that can be filled by a **pursuit**, and a per-turn **muster limit** of **retinues**. The exact values per tier are below:

{{TABLE:settlements}}

### Settlement Wards

**Settlements** contain **1 settlement ward** for each tier of settlement your settlement is. **Villages** have **1 central hub**, which all other **settlement wards** must be **adjacent** (range 1)**,** **towns** have 2 settlement wards, **cities** have 3, and a **metropolis** can have 4. The only exception is a **hamlet**, which is a settlement that can only contain **husbandry pursuits**, but can contains **3 settlement wards**. 
...
## Infrastructure
 	...
 	Infrastructure is the foundation of your settlements. Each infrastructure you have active provides a effect that affects your entire empire and settlements.
...
**Territory** is the smallest unit of land. To move from one territory to another, you move **range 1**. To be **within** a territory, you are **range 0**; **adjacent**, **range 1**; **next to**, **range 2**.
 	...
 	**Territories** have modifiers based on the type of hex they are (grassland, forest, tundra, wetlands, water, & mountain), and can either be **uncontrolled**, **contested**, or **controlled**. **Territory** begins as **uncontrolled**. By **chartering** **settlements**, you can **control territories**. All **settlements** have a **reach X**, adding all **territory** to your **province** within the **range** of your **settlements** equal to your **reach** characteristic. However, if another **player’s settlement reach** is within **reach** of any **territory** you **control**, it is **contested**.
 	...
 	To **border** another **player**, your **reach** must be **within** or **adjacent** to another **player’s territory**. 
...
*Tracking tip: keep three small piles — Treasury, Revenue, Upkeep. When you build something with income or upkeep, add it to the matching pile; when it's damaged or an army loses retinues, subtract it. Then each turn it's just Treasury + Revenue − Upkeep, with no recounting.*
...
An inactive pursuit gives no innate or mastery effect, and anything that needed it as a prerequisite loses its mastery. An inactive piece is restored at any future Upkeep Step where you can pay its upkeep. Build Timers halt while you are Insolvent.

If you've cut everything and a debt still remains, you are **Bankrupt**: your Treasury goes negative by the unpaid amount (a Debt Marker). While Bankrupt you cannot spend gold or be Extorted, and Build Timers stay halted; Revenue pays down the debt first. When your Treasury reaches 0 or above, Bankruptcy ends. *(If you are Vassalized while Bankrupt, the debt is forgiven and your Suzerain pays your Treasury's until the balance is a minimum of 0.)*

## Trade & Income

### Who Can Trade & Income

 **Who Can Trade:** **players** need to have **active dirt road infrastructure** and a **signed trade agreement** to participate in **trade**.
...
- When you **sign** a **trade agreement**: The **player** who did *not* **perform** the **sign treaty action** gets **trade income** next time they are **host**. Afterwards, both get **trade income** each turn either of them are Host.
...
- **Taxes:** At start of each winter phase, gain **tax income** equal to your **settlement** total, after modifiers.
- **Trade:** See above.
...
**Armies** are a collection of **retinues** that exist on the **realm**. **Armies** are the target of **move** **actions**. **Armies** consist of up to {{VAL:ARMY_MAX_RETINUES}} **retinues** with up to a single piece of **equipment** from each type (**armor**, **weapons**, **shields**, & Retinue).

 	**Armies** cost **upkeep** that must be spent in the **empire phase**. To calculate your **upkeep** for your **armies**, sum the count of armies by their **retinue type cost**, then subtract any **upkeep modifiers**.

 	Note: For example, Net Upkeep = Retinue Count * (Retinue Cost - Upkeep Modifiers)

If an army has been **engaged** in a **battle** or **lay siege action** started by another **player’s army**, both **armies** cannot be the **target** of any **actions** until the **battle** or **lay siege** has been resolved. 

### Recruiting an Army

When you **perform** a **muster action**, if you do not have the maximum amount of **armies** you are allowed, you may place an **army** is **within** (range 0) a **settlement** you **control** that is not being **sieged** by another **player**. When you do, you may **muster** an amount of **retinues** according to the **muster** **limit** of the **settlement**, **pay** the appropriate **cost** according to the **type** of **retinue** you are eligible to purchase, select their **equipment** based on the available equipment you have to choose from, and set a **muster timer {{VAL:TIMERS.Muster Timer.default}}**.
...
Form the line. Each side places up to {{VAL:FRONT_LINE_MAX}} retinues in its front line — one Strike die each — and may keep up to {{VAL:RESERVE_MAX}} in reserve to replace losses as they fall. An army of {{VAL:ARMY_MAX_RETINUES}} keeps the remainder in camp; between Skirmishes, camp and reserves refill the front line and reserve back to their maximums.
...
Panic check. If the side about to Strike back took {{VAL:PANIC_CASUALTY_THRESHOLD}} or more casualties this Skirmish, it first takes a Panic check: roll its Morale, up to {{VAL:MORALE_DICE_MAX}} dice; failures are casualties. Immune Panic auto-passes. A check ever modified to 7 or more Routs the whole army.
...
3. If attacked while sieging — if you **fallback**, the **siege** is broken and the **siege timer** is removed from play.
...
**Outlaw Country** is a collection of **territory** in your **starting settlement region** that you cannot **control**. At the beginning of the game, you’ll demarcate {{VAL:OUTLAW_COUNTRY_START}} territories in your **starting settlement region**, all no more than **range 1** of at least 1 other **Outlaw Country territory** and at least **range {{VAL:OUTLAW_BUFFER_RANGE}}** from any **settlement**.

 If this is not possible, place them **range 2** from your **capital settlement**.

### Spawning Bandit Camps

At the beginning of Bandit Mechanic Phase in Spring: Place a **bandit camp** with {{VAL:BANDIT_CAMP_START}} Bandits (retinues) in each **player’s** Outlaw Country. 

 *Note: if your outlaw country is selected to receive another bandit camp, but there is no room to place one, instead, each existing Bandit Camp increases in size according to the current Era.*
