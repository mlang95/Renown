...
def build(out_path):
    ...
    # ── 17) ACTIONS (data-driven from rd.ACTIONS) ──
    if hasattr(rd, "ACTIONS"):
        for n, a in rd.ACTIONS.items():
            dom = a.get("domain", "")
            cost = a.get("cost", "") or "None"
            req = a.get("requires", "")
            eff = a.get("effect", "")
            endo = a.get("endorsed", "")
            body = f"{dom} action. Cost: {cost}."
            if req:
                body += f" Requires: {req}."
            body += f" {eff}"
            if endo:
                body += f" Endorsed: {endo}"
            link = wlink(n, "actions-ref.html")
            lines.append(entry(n, f"{link}: {body}"))
        lines.append(entry("actions", wlink("Actions", "actions-ref.html") +
                     ": every Envoy action by Domain. Prowess (Move/Declare War/Demand Tribute), "
                     "Cunning (Intercept Caravan/Foster Rebellion/Raze/Destabilize), Piety (Spread "
                     "Gospel/Send Missionaries/Tithe/Convert/Crusade), Industry (Build/Repair/Pursue/"
                     "Charter), Diplomacy (Sign Treaty/Negotiate/End Treaty)."))

    # ── 18) TREATIES (data-driven from rd.TREATIES) ──
    if hasattr(rd, "TREATIES"):
        for n, t in rd.TREATIES.items():
            era = t.get("era", "")
            era_s = f" (unlocks: {era})" if era and era != "Any" else ""
            lines.append(entry(n, wlink(n, "treaties-ref.html") +
                         f" Treaty{era_s}. {t.get('effect','')}"))
        lines.append(entry("treaties", wlink("Treaties & Alliances", "treaties-ref.html") +
                     ": " + ", ".join(rd.TREATIES.keys()) + ". Signed via Diplomacy."))
        lines.append("alliances | See 'treaties'. Military Alliances unlock at Eminence, Defensive at Zenith.")

    # ── 19) EDICTS (data-driven from rd.EDICTS) ──
    # Some Edict names collide with glossary/action keys (Monument, Wonder,
    # Vassalize). Key those with an "edict " prefix so they don't overwrite the
    # richer glossary/action entry; unique names (Wealth, Living Saints) keep
    # their plain key.
    if hasattr(rd, "EDICTS"):
        _existing = {ln.split("|", 1)[0].strip().lower() for ln in lines}
        for n, ed in rd.EDICTS.items():
            txt = wlink(n, "edicts-ref.html") + f" Edict ({ed.get('type','')}). {ed.get('requirement','')}"
            key = n if n.lower() not in _existing else f"edict {n}"
            lines.append(entry(key, txt))
        lines.append(entry("edicts", wlink("Edicts", "edicts-ref.html") +
                     ": the win paths. " + ", ".join(rd.EDICTS.keys()) +
                     ". Completing one raises Renown by 1; most-completed wins."))

    # ── 20) ECONOMY: upkeep tracks + costs (data-driven) ──
    if hasattr(rd, "UPKEEP_TRACKS"):
        for track, desc in rd.UPKEEP_TRACKS.items():
            lines.append(entry(f"{track.lower()} upkeep",
                         wlink(f"{track} upkeep", "economy-ref.html") + f": {desc}"))
        lines.append(entry("upkeep", wlink("Upkeep", "economy-ref.html") +
                     ": three separate tracks — Pursuit (fixed by type), Army (retinue count x cost - "
                     "reductions), and Infrastructure (per-settlement). See 'pursuit upkeep', "
                     "'army upkeep', 'infrastructure upkeep'."))
    if hasattr(rd, "COSTS"):
        lines.append(entry("costs", wlink("Costs", "economy-ref.html") +
                     ": Cunning/Industry actions cost 2000 gold; Piety costs Doubt 1; Prowess and "
                     "Diplomacy are free. Tax = 500/tier/turn (Winter). Trade = 100 x Craft per agreement."))
        for k in ("Sack", "Tax per tier", "Trade Income"):
            if k in rd.COSTS:
                lines.append(entry(k.lower(), wlink(k, "economy-ref.html") + f": {rd.COSTS[k]}"))
