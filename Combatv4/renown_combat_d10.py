...
# ── Keyword constants ─────────────────────────────────────────────────────
# Rename a keyword here and it renames everywhere (GLOSSARY keys, tags, cards).
from dice_config import FACES, FOCUSED_THR, ROUT_THR, CAP_THR, PARRY_BASE, RECOVER_BASE
    ...
    for i in range(strikes):
        ...
        if is_shatter:
            ...
            if save_target > 6:
                ...
                if "Poison" in atk_tags:
                    roll = random.randint(1, FACES)
                    failed = (roll >= FOCUSED_THR)
                else:
                    failed = False
                    roll = None
            else:
                roll = random.randint(1, FACES)
                if "Poison" in atk_tags and roll >= FOCUSED_THR:
                    failed = True
                else:
        ...
        if failed:
            if "Parry" in def_tags:
                if random.randint(1, FACES) >= PARRY_BASE:
                    continue
            ...
            for t in def_tags:
                if t == "Regenerate" or t == "Regenerate 6":
                    regen_threshold = RECOVER_BASE if regen_threshold is None else min(regen_threshold, RECOVER_BASE)
                elif t == "Regenerate 5":
                    regen_threshold = RECOVER_BASE - 1 if regen_threshold is None else min(regen_threshold, RECOVER_BASE - 1)
                elif t == "Regenerate 4":
                    regen_threshold = RECOVER_BASE - 2 if regen_threshold is None else min(regen_threshold, RECOVER_BASE - 2)
            if regen_threshold is not None:
                ...
                if not saved and "Regenerate Reroll" in def_tags:
                    saved = random.randint(1, FACES) >= regen_threshold
                if saved:
                    continue
            casualties += 1
