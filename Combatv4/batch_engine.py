...
def pack_side(loadouts, is_attacker):
    ...
    # Deadly/Cleave/Destroy Shield (first/normal) for strike resolution.
    ...
    for t in [CLEAVE, DESTROY_SHIELD]:
        ...
        P[f"first_{key}"]  = np.array([t in a.tags_first  for a in armies], dtype=np.bool_)
        P[f"normal_{key}"] = np.array([t in a.tags_normal for a in armies], dtype=np.bool_)
    # Per-slot crit_floor (Focused threshold): lowest `Crit N` tag present, else FOCUSED_THR.
    # Mirrors vectorized_combat._crit_floor so Ministry etc. retune in renown_data, not here.
    def _crit_floor(tagset):
        floors = [int(m.group(1)) for t in tagset
                  for m in (re.match(r"^Crit (\d+)$", str(t)),) if m]
    ...
    P["normal_Planishing"] = np.array([bool(a.planishing) for a in armies], dtype=np.bool_)
...
if _HAVE_NUMBA:
    # Strike/save kernels now come from combat_kernel (shared). Only _shake_kernel
    # remains batch-local (morale shaking is identical in both engines and unchanged).
...
def _roll_strikes_batch(rng, n, target_th, front_line, has_deadly, has_cleave,
                        has_destroy, def_has_shield, def_shield_destroyed, def_shield_immune,
                        crit_floor, has_dual_wield=None):
    """Batched strikes via the SHARED kernel (combat_kernel._strikes_kernel) — identical
    per-die logic to vectorized_combat. Per-slot arrays (n,):
      has_deadly  : attacker's strike carries Deadly (was 'Shatter Armor')
      has_cleave  : attacker's strike carries Cleave
      has_destroy : attacker can Destroy Shield (proc = a Focused strike, roll >= FOCUSED_THR)
    ...
    af = auto_fail.astype(np.bool_).copy()
    ap = auto_pass.astype(np.bool_).copy()
    cf = np.asarray(crit_floor, dtype=np.int64)
    if has_dual_wield is None:
        has_dual_wield = np.zeros(n, dtype=np.bool_)
    # The shared kernel takes SCALAR has_deadly/has_cleave/crit_floor/reroll. Batch slots vary
    # per-element, so we use the DUAL kernel (per-slot deadly/cleave arrays) for the general path
    # and pass a per-slot crit_floor by widening the kernel call across the (already per-slot)
    # masks. crit_floor varies per slot, but the kernel signature takes one int — so we resolve
    # it the same way vectorized does for the rare Crit build: split by the distinct crit_floor
    # values present (almost always just {6}, sometimes {6,4}). This keeps one shared kernel.
    strikes = np.zeros(n, dtype=np.int32)
    deadly = np.zeros(n, dtype=np.int32)
    procs = np.zeros(n, dtype=np.int32)
    hd = np.asarray(has_deadly, dtype=np.bool_)
    hc = np.asarray(has_cleave, dtype=np.bool_)
    for floor_val in np.unique(cf):
        m = (cf == floor_val)
        if not m.any():
            continue
    ...
    destroyed_shield = can_destroy & (procs > 0) & (~def_shield_destroyed)
    return strikes, deadly, destroyed_shield


def _roll_saves_batch(rng, n, save_target, n_strikes, deadly_strikes,
    ...
    Per-slot arrays (n,):
      deadly_strikes   : count of Deadly strikes among the n_strikes (resolve at AP-5, parry/
                         recover on natural-6 only)
      parry_mask       : defender can Parry
      regen_thr        : defender Recover threshold (0 = none; already incl. Serrated + the
                         opponent's Rend from _precompute_regen_parry)
      atk_unstoppable  : attacker Unstoppable (+1 to defender parry threshold)
    ...
    (def_can_parry_shatter / regen_reroll_mask retained for call-site compat; the shared kernel
     handles Deadly parry-on-6 intrinsically and reroll is deprecated.)"""
    ...
    # Defaults for optional per-slot arrays.
    if atk_unstoppable is None:    atk_unstoppable = np.zeros(n, dtype=np.bool_)
    if def_riposte is None:        def_riposte = np.zeros(n, dtype=np.bool_)
    if def_parry_improved is None: def_parry_improved = np.zeros(n, dtype=np.bool_)
    if atk_is_ranged is None:      atk_is_ranged = np.zeros(n, dtype=np.bool_)
    if def_planishing is None:     def_planishing = np.zeros(n, dtype=np.bool_)
    if atk_has_deflect is None:    atk_has_deflect = np.zeros(n, dtype=np.bool_)
    if atk_ignores_tempered is None: atk_ignores_tempered = np.zeros(n, dtype=np.bool_)
    if def_fat is None:            def_fat = np.zeros(n, dtype=np.int64)

    # ── SHARED primitive math (combat_primitives) — same code vectorized calls ──
    ...
    regen_thr_arr = _cp.build_regen_thr(n, regen_thr, def_fat)
    pmask = _cp._as_bool_arr(parry_mask, n)
    riposte_mask = _cp.build_riposte_mask(n, def_riposte, deflect)

    n_strikes_arr = np.asarray(n_strikes, dtype=np.int64)
    deadly_arr = np.asarray(deadly_strikes, dtype=np.int64)
    poison = _cp._as_bool_arr(atk_poison, n)

    any_parry = bool(pmask.any())
    any_regen = bool((regen_thr_arr > 0).any())

    # RNG draw order MUST match vectorized: rolls, parry_rolls, regen_rolls.
    rolls = rng.integers(1, FACES + 1, size=(n, max_strikes), dtype=np.int8)
    parry_rolls = (rng.integers(1, FACES + 1, size=(n, max_strikes), dtype=np.int8)
                   if any_parry else np.zeros((n, max_strikes), dtype=np.int8))
    regen_rolls = (rng.integers(1, FACES + 1, size=(n, max_strikes), dtype=np.int8)
                   if any_regen else np.zeros((n, max_strikes), dtype=np.int8))
...
def run_batch_random(pairs, n_runs=50, seed=2026, max_skirmishes=40, mode="random",
    ...
    a_break_used = np.zeros(N, dtype=bool); b_break_used = np.zeros(N, dtype=bool)  # Rally
    a_panic_used = np.zeros(N, dtype=bool); b_panic_used = np.zeros(N, dtype=bool)  # Resolute
    ...
    for sk in range(max_skirmishes):
        ...
        a_negtemp = tile(Pa[f"{skl}_NegateTempered"]); b_negtemp = tile(Pb[f"{skl}_NegateTempered"])
        a_planish = tile(Pa[f"{skl}_Planishing"]); b_planish = tile(Pb[f"{skl}_Planishing"])
        ...
        # Cap (base + improving + fatigue) at CAP_THR; then worsening mods apply and can reach FACES+1 (auto-fail).
        ...
        a_th_pre = np.minimum(a_to_hit + a_th_improve + a_fat, CAP_THR)
        b_th_pre = np.minimum(b_to_hit + b_th_improve + b_fat, CAP_THR)
        # Blunder: clamp to CAP_THR+ at this step (then worsening mods can push beyond).
        a_th_pre = np.where(a_tripped, np.maximum(a_th_pre, CAP_THR), a_th_pre)
        b_th_pre = np.where(b_tripped, np.maximum(b_th_pre, CAP_THR), b_th_pre)
        ...
        # per side per skirmish. Each initiative-step bundle was already front(10)-capped upstream;
        # the rear ("at camp") cannot die this skirmish. Matches vectorized_combat.
        FIELD_CAP = vc.FRONT_CAP + vc.RESERVE_CAP
        a_cas = np.minimum(a_cas + a_rip_cas, FIELD_CAP)
        b_cas = np.minimum(b_cas + b_rip_cas, FIELD_CAP)
        ...
        # Identical to vectorized_combat: PANIC (>5 net combat) -> BREAK (exhausted) -> ROUT.
        import combat_morale as _cm
        a_steadfast = tile(np.array([(("Steadfast" in t) or ("Immune Panic" in t))
        ...
        _aM = _cm.resolve_side_morale(
            ...
            a_netc, a_shaking, a_shake_bonus, a_unbreak_eff, a_steadfast,
            a_cause, a_waver, a_shake, a_rout,
            field_budget=a_clost, field_cap=(vc.FRONT_CAP + vc.RESERVE_CAP),
            morale_cap_mask=a_mcap, zealot_mask=a_zealot,
            steadfast_firstpass_mask=a_rally, break_used=a_break_used,
            resolute_firstpass_mask=a_resolute, panic_used=a_panic_used)
        _bM = _cm.resolve_side_morale(
            rng, N, _shake_batch, vc.SHAKE_CAP,
            b_size, b_end, b_fat, active, ending,
            b_netc, b_shaking, b_shake_bonus, b_unbreak_eff, b_steadfast,
            b_cause, b_waver, b_shake, b_rout,
            field_budget=b_clost, field_cap=(vc.FRONT_CAP + vc.RESERVE_CAP),
            morale_cap_mask=b_mcap, zealot_mask=b_zealot,
            steadfast_firstpass_mask=b_rally, break_used=b_break_used,
            resolute_firstpass_mask=b_resolute, panic_used=b_panic_used)
        a_size = _aM["size"]; a_cause = _aM["cause"]
        a_waver = _aM["cas_panic_total"]; a_shake = _aM["cas_break_total"]; a_rout = _aM["cas_rout_total"]
        a_exh = _aM["exhausted"]; a_break_used = _aM["break_used"]; a_panic_used = _aM["panic_used"]; a_slost = _aM["break_lost"]; a_wvlost = _aM["panic_lost"]
        b_size = _bM["size"]; b_cause = _bM["cause"]
        b_waver = _bM["cas_panic_total"]; b_shake = _bM["cas_break_total"]; b_rout = _bM["cas_rout_total"]
        b_exh = _bM["exhausted"]; b_break_used = _bM["break_used"]; b_panic_used = _bM["panic_used"]; b_slost = _bM["break_lost"]; b_wvlost = _bM["panic_lost"]
        # Heal next skirmish reads COMBAT-only casualties (shake/panic/rout can't be healed).
        a_prev_combat = a_clost.copy(); b_prev_combat = b_clost.copy()
        a_prev_cas = a_scas + a_slost + a_wvlost; b_prev_cas = b_scas + b_slost + b_wvlost
...
def _shake_batch(rng, n, field_size, shaking_value, mask):
    ...
    rolls = rng.integers(1, FACES + 1, size=(n, max_field), dtype=np.int8)
    if _HAVE_NUMBA:
...
def _warm_numba_kernels():
    ...
    try:
        ...
        n = 4
        # SHARED _strikes_kernel (combat_kernel) — full current signature.
        rolls = rng.integers(1, 7, size=(n, 20), dtype=_np.int8)
        crolls = rng.integers(1, 7, size=(n, 20), dtype=_np.int8)
        fl = _np.full(n, 3, _np.int64); th = _np.full(n, 4, _np.int64)
        ...
        rrolls = rng.integers(1, 7, size=(n, 5), dtype=_np.int8)
        ns = _np.full(n, 3, _np.int64); dl = _np.zeros(n, _np.int64)
        sc = _np.full(n, 4, _np.int64); dsc = _np.full(n, 6, _np.int64)
        aps = _np.zeros(n, _np.bool_); pois = _np.zeros(n, _np.bool_)
        pm = _np.zeros(n, _np.bool_); pthr = _np.full(n, 5, _np.int64)
        rt = _np.zeros(n, _np.int64); ripm = _np.zeros(n, _np.bool_); rip5 = _np.zeros(n, _np.bool_)
        _shared_saves_kernel(srolls, prolls, rrolls, ns, dl, sc, dsc, aps,
                             pois, pm, pthr, rt, ripm, rip5, bool(PARRY_BEFORE_SAVE), False)
        # _shake_kernel (batch-local, unchanged)
...
def _batch_block_worker(args):
    ...
    import os as _os
    _dbg = _os.environ.get("RENOWN_MEM_DEBUG")
    _t0 = _proc = None
    if _dbg:
        try:
            import psutil as _ps, time as _t
            _proc = _ps.Process(); _t0 = _t.perf_counter()
        except Exception:
            _proc = None
    # SINGLE real call — debug only wraps it (never duplicates args, so kwargs can't drift).
    res = run_batch_random(block_pairs, n_runs=n_runs, seed=seed, mode=mode,
                           return_first_tactics=True)
    if _dbg and _proc is not None:
        import time as _t
        _gb = 1024**3
        _slots = len(block_pairs) * n_runs
        _rss = _proc.memory_info().rss
        print(f"[MEM] block: {len(block_pairs)} pairs x {n_runs} = {_slots:,} slots "
              f"| RSS={_rss/_gb:.2f}GB | bytes/slot~={_rss/max(1,_slots):,.0f} "
              f"| {_t.perf_counter()-_t0:.1f}s", flush=True)
    return res


def auto_slot_budget(n_workers, bytes_per_slot=1400, safety=0.35, floor=40_000, cap=400_000):
    ...
    try:
        ...
        free = 2 * 1024**3
    raw = int(free * safety / (bytes_per_slot * max(1, n_workers)))
    budget = max(floor, min(cap, raw))
    import os as _os
    if _os.environ.get("RENOWN_MEM_DEBUG"):
        gb = 1024**3
        clamp = "raw" if budget == raw else ("CAP" if budget == cap else "FLOOR")
        print(f"[MEM] auto_slot_budget: psutil={have_psutil} "
              f"free_used={free/gb:.1f}GB workers={n_workers} bytes/slot={bytes_per_slot} "
              f"safety={safety} -> raw={raw:,} clamped={budget:,} ({clamp})", flush=True)
