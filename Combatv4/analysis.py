...
def tag_synergy_matrix(df, tags=None, min_support=30):
    ...
    bwr = df.groupby("a_name").agg(
        win_rate=("a_win_rate", "mean"),
        tags=("a_tags", "first")
    ).reset_index()
    
    # Convert categorical tags to string type first, then handle blanks and sets
    bwr["tags"] = bwr["tags"].astype("string").fillna("")
    bwr["tagset"] = bwr["tags"].apply(lambda s: set(t for t in s.split(",") if t))
