...
set PUSH_REPO=1
REM REPO_DIR  : the git working tree to push. Default = CE_ROOT, which keeps CE
REM   as its OWN repo, separate from Combatv3/D6. If instead CE is tracked
REM   inside the parent Game repo, point this at that root - but note that
REM   pushing the Game root re-couples CE with Combatv3, which this build
REM   otherwise never touches.
set REPO_DIR=%CE_ROOT%
